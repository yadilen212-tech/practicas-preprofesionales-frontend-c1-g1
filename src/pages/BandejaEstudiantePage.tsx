...
// Mismo mapa que STATUS_TONE en components/StatusBadge.tsx, copiado acá
// porque esta bandeja no importa el componente Fila de las otras tres.
...
/**
 * Bandeja de revisión del estudiante: el estado de los documentos que subió,
 * pendientes o ya revisados, en un solo lugar.
 * copiado de la bandeja del tutor, luego lo refactorizo — esta pantalla no
 * tiene fila propia en components/, la tabla se quedó inline.
 */
export function BandejaEstudiantePage() {
  // ---- estado ----
  ...
  return (
    <>
      <PageHeader
        title="Bandeja de mis documentos"
        subtitle="Documentos que subiste, pendientes o ya revisados por la coordinación."
      />
        ...
        >
          <div className="flex flex-col">
            {itemsPagina.map((item) => {
              ...
              const estadoAccion = accionando[item.id] ?? null
              const ocupado = estadoAccion === 'aprobando' || estadoAccion === 'rechazando'
              return (
                <div
                  key={item.id}
                  className="flex min-h-row flex-col justify-center gap-1 px-[18px] py-2.5 transition-colors hover:bg-well sm:flex-row sm:items-center sm:gap-3"
                >
                    ...
                    {!resuelto ? (
                      <>
                        <Button
                          ...
                          disabled={ocupado}
                        >
                          {estadoAccion === 'aprobando' ? 'Aprobando…' : 'Aprobar'}
                        </Button>
                        <Button
                          type="button"
                          size="sm"
                          ...
                          disabled={ocupado}
                        >
                          {estadoAccion === 'rechazando' ? 'Rechazando…' : 'Rechazar'}
                        </Button>
                      </>
                    ) : (
