...
describe('bandejaEstudiante api', () => {
  ...
  it('aprueba un item con nota opcional', async () => {
    ...
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/1/aprobar', {
      method: 'PATCH',
  ...
  it('rechaza un item con nota obligatoria', async () => {
    ...
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/1/rechazar', {
      method: 'PATCH',
  ...
  it('cuenta los items pendientes', async () => {
    ...
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/pendientes/total')
  })

  it('busca por texto libre, codificando la consulta', async () => {
    vi.mocked(api).mockResolvedValue([])
    await buscarBandejaPorTexto('ana pérez')
    expect(api).toHaveBeenCalledWith(`/estudiante/bandeja?q=${encodeURIComponent('ana pérez')}`)
  })

  it('marca un item como leído', async () => {
    vi.mocked(api).mockResolvedValue({ id: 1 })
    await marcarComoLeido(1)
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/1/marcar-leido', { method: 'PATCH' })
  })

  it('archiva un item', async () => {
    vi.mocked(api).mockResolvedValue({ id: 1 })
    await archivarItem(1)
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/1/archivar', { method: 'PATCH' })
  })

  it('reabre un item', async () => {
    vi.mocked(api).mockResolvedValue({ id: 1 })
    await reabrirItem(1)
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/1/reabrir', { method: 'PATCH' })
  })

  it('lista solo los items con el estado indicado', async () => {
    vi.mocked(api).mockResolvedValue([])
    await listarBandejaPorEstado('SUBMITTED')
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja?estado=SUBMITTED')
  })

  it('lista la bandeja paginada', async () => {
    vi.mocked(api).mockResolvedValue([])
    await listarBandejaPaginada(2, 10)
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja?pagina=2&tamano=10')
  })
  ...
  it('obtiene el resumen numérico de la bandeja', async () => {
    ...
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/resumen')
  })

  it('fija un item', async () => {
    vi.mocked(api).mockResolvedValue({ id: 1 })
    await fijarItem(1)
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/1/fijar', { method: 'PATCH' })
  })

  it('desfija un item', async () => {
    vi.mocked(api).mockResolvedValue({ id: 1 })
    await desfijarItem(1)
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/1/desfijar', { method: 'PATCH' })
  })

  it('lista solo los items fijados', async () => {
    vi.mocked(api).mockResolvedValue([])
    await listarBandejaFijados()
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja?fijados=true')
  })

  it('lista solo los items archivados', async () => {
    vi.mocked(api).mockResolvedValue([])
    await listarBandejaArchivados()
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja?archivados=true')
  })
  ...
  it('reenvía la notificación de un item', async () => {
    ...
    expect(api).toHaveBeenCalledWith('/estudiante/bandeja/1/reenviar-notificacion', { method: 'POST' })
  })
})
